  ...
  if (!stockForRw.ok) {
    ...
    if (!source) {
      ...
      const rwCreate = await http('POST', '/documents/rw', {
        recipient: 'Runda 2 UAT',
        items: [{ product_id: source.product_id, quantity: 1 }],
      });
      ...
      if (rwConfirm.ok) {
        ...
        if (rwConfirm.status === 400 && detail.includes('strefie kompletacji')) {
          row('rw_confirm_business_rule', 'OK', 'RW poprawnie zablokowane bez stanu w strefie kompletacji');
        } else {
